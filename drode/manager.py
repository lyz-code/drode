...
class DeploymentManager:
    ...
    def _print_aws_autoscaling_group_info(self, autoscaling_name):
        ...
        try:
            ...
            print('Active LaunchConfiguration: {}'.format(
        ...
        for instance_data in autoscaling_group['Instances']:
            ...
            instances_data.append([
                ...
                '{}/{}'.format(
                    instance_data['HealthStatus'],
                    instance_data['LifecycleState'],
                ),
    ...
    def builds_url(self):
        ...
        return '{}/api/repos/{}/builds'.format(
            self.drone_url,
            self.config.get(
                'projects.{}.pipeline'.format(self.config.project)
            ),
        )
    ...
    def get(self, url, method='get'):
        ...
        if method == 'post':
            response = requests.post(
                url,
                headers={
                    'Authorization': 'Bearer {}'.format(self.drone_token)
                },
            )
        else:
        ...
        if response.status_code == 200:
            ...
            log.error(
                '{} error while trying to access {}'.format(
    ...
    def last_success_build_info(self, branch='master'):
        ...
        for build in history:
            if build['status'] == 'success' and \
                    build['target'] == branch and \
                    build['event'] == 'push':
                return build
        log.error(
            'There are no successful jobs with target branch {}'.format(branch)
        )
        sys.exit(1)
    ...
    def status(self):
        ...
        for environment in ['Production', 'Staging']:
            ...
            try:
                self._print_aws_autoscaling_group_info(self.config.get(
    ...
    def wait(self, build_number=None):
        ...
        while True:
            ...
            try:
                ...
                log.info(
                    'Job #{} has not started yet'.format(build_number)
                )
            return True
